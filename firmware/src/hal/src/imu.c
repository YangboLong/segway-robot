...
void imu6Init(void)
{
    ...
    i2cdevInit(I2C3_DEV); // PA8: SCL; PC9: SDA
    mpu6500Init(I2C3_DEV);
    if (mpu6500TestConnection() == true) {
        DEBUG_PRINTF("MPU9250 I2C connection [OK].\n");
    } else {
        DEBUG_PRINTF("MPU9250 I2C connection [FAIL].\n");
    }
    ...
    if (ak8963TestConnection() == true) {
        ...
        DEBUG_PRINTF("AK8963 I2C connection [OK].\n");
    } else {
        DEBUG_PRINTF("AK8963 I2C connection [FAIL].\n");
    }
#endif
    ...
    if (testStatus) {
        while (xTaskGetTickCount() - startTick < IMU_VARIANCE_MAN_TEST_TIMEOUT) {
            ...
            if (gyroBias.isBiasValueFound) {
                DEBUG_PRINTF("Gyro variance test [OK]\n");
                break;
            }
        }
        ...
        if (gyroBias.isBiasValueFound) {
            ...
            if ((fabsf(roll) < IMU_MAN_TEST_LEVEL_MAX)
                    && (fabsf(pitch) < IMU_MAN_TEST_LEVEL_MAX)) {
                DEBUG_PRINTF("Acc level test [OK]\n");
                testStatus = true;
            } else {
                DEBUG_PRINTF("Acc level test Roll:%0.2f, Pitch:%0.2f [FAIL]\n", roll, pitch);
                testStatus = false;
            }
        } else {
            DEBUG_PRINTF("Gyro variance test [FAIL]\n");
            testStatus = false;
        }
    }
